function obj = LS_skeleton_extraction_3D(obj,varargin)
%%  依赖关系判断
if obj.syset.flags.read_flag_vox~=1
    error('voxelization has not been processed yet!')
end
%%  default values
default_test_flag= 1;       % 测试标志
default_fit_mode = 2;       % 拟合模式，1.直接拟合y=f(x) 2.参数化拟合x=f(t),y=f(t)
...
for i = 1:length(obj.LS_Vox)
    ...
    obj.LS_Vox(i).curve = cp_extract_cspline(x1,y1,0.5,'mode',2);    %   这里其实还是当作二维曲线来拟合的，如果后续想拟合三维曲线，需要编写对应的方程，并提前做异常数据剔除
        case 2
            ...
            obj.LS_Vox(i).curve_z = cp_extract_cspline([1:length(z2)]'/length(z2),z2,1-1e-6,'mode',2);
            if test_flag
                tempt=0:1e-5:1;
                fx=obj.LS_Vox(i).curve_x.curve_model;
                fy=obj.LS_Vox(i).curve_y.curve_model;
                fz=obj.LS_Vox(i).curve_y.curve_model;
                subplot(2,2,1)
                plot3(x1(1),y1(1),z1(1),'ro',x1,y1,z1,'k-',x2(1),y2(1),z2(1),'bo',x2,y2,z2,'r-',fx(tempt),fy(tempt),fz(tempt),'b--')
                view([0 0 1]);
                subplot(2,2,2)
                plot3(x1(1),y1(1),z1(1),'ro',x1,y1,z1,'k-',x2(1),y2(1),z2(1),'bo',x2,y2,z2,'r-',fx(tempt),fy(tempt),fz(tempt),'b--')
                view([0 1 0]);
