...
for i = 1:2:numel(varargin)
    ...
    if strcmpi(prm, 'Color')
        if ischar(val)
            ...
            %   2024-01-23  Martin if color is 'random' it will not works, I
            %               I specified the color here.
            if strcmp(val, 'random')
                temp_list = hFig.UserData.colors;
                val=cell2mat(temp_list(round(rand()*(size(temp_list,1)-1)+1),3));
            end
